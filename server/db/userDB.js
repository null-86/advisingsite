    ...
    return new Promise((resolve, reject) => {
        connection.query(`SELECT aa.studentCwid, concat(e.fName, ' ', e.mInit, ' ', e.lName) as username, e.email FROM tre_database.advisorAdvisee as aa 
                            join tre_database.endUser as e on aa.studentCwid = e.cwid 
                                where aa.advisorCwid = '${advID}' and term = '${term}';`, (err, results) => {
            if (err) {
                return reject(err)
            }
            if(results.length == 0) {
                            ...
                            join tre_database.classSchedule b on a.studentCwid = '${cwid}' and a.scheduleId = b.scheduleId join tre_database.scheduleAgreement  c on c.scheduleId = b.scheduleId 
                                join tre_database.courseSection  d on d.crn = b.crn join tre_database.course  e on e.subject= d.subject and e.courseNum = d.courseNum 
                                    join tre_database.sectionSetting  f on f.crn = d.crn 
                                        join tre_database.advisorAdvisee g on g.studentCwid = a.studentCwid and g.term = a.term
                                            join tre_database.endUser h 
                                                where c.studentAgree = 1 and c.advisorAgree = 1 and h.cwid = g.advisorCwid;`, 
                                                    (err, res) => {
                                                        if (err) {
                                                            reject(err);
                                                            return;
...
updateZoom = (cwid, zmLink) => {
    return new Promise((resolve, reject) => {
        connection.query(`Select advisorCwid FROM tre_database.zoomInfo 
                             where advisorCwid = '${cwid}';`, (err, value) => {
            if (err) {
                reject(err);
                return;
            }
             ...
             if (value.length == 0) {
               connection.query(`INSERT INTO tre_database.zoomInfo (advisorCwid, zoomLink) VALUES ('${cwid}', '${zmLink}');`, (err, val) => {
                    if (err) {
                        reject(err);
                        return;
                    }
             ...
             connection.query(`UPDATE tre_database.zoomInfo SET zoomLink = '${zmLink}' WHERE advisorCwid = '${cwid}'`, (err, value) => {
                if (err) {
                    reject(err);
                    return;
                }
...
getZoomLink = (cwid) => {
    return new Promise((resolve, reject) => {
        connection.query(`SELECT a.advisorCwid, a.zoomLink, concat(b.fName, ' ', b.lName) as advName, b.email FROM tre_database.zoomInfo a 
                            join tre_database.endUser b on a.advisorcwid = b.cwid WHERE advisorCwid=${cwid};`, (err, result) => {
            if (err) {
                reject(err);
                return;
            }
